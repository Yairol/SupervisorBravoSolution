...
namespace SupervisorBravo.Web.Controllers.Dixells.CoolingCoercion
{
    public class CoolingCoercionController : Controller
    {
        ...
        public async Task<IActionResult> Dixells()
        {
            ...
            {
                // 👉 Ahora obtenemos directamente el último valor
                var ultimoDato = await ((ITemperatureRepository)_dixellRepository)
                    .GetLastTemperatureByDixell(device.Id);
            ...
            if (dixellsList == null || dixellsList.Count == 0)
            {
                return NotFound();
            }
